...
-- name: ListPaymentsForParentSchool :many
SELECT p.id, p.amount_cents, p.currency, p.method, p.status, p.reference, p.approved_at, p.created_at
FROM payments p
WHERE p.parent_id = $1
  AND p.school_id = $2
ORDER BY p.created_at DESC
...
-- name: ListInvoicesForParentSchool :many
SELECT i.id, i.amount_cents, i.currency, i.status, i.due_date, i.paid_at, i.created_at
FROM invoices i
WHERE i.parent_id = $1
  AND i.school_id = $2
ORDER BY i.created_at DESC
