...
// ErrSubscriptionRequired is returned by the Super App live path when the linked
// parent has no active subscription or trial. Uses a distinct type (not ErrAccessDenied)
// so the handler can surface a specific SUBSCRIPTION_REQUIRED error code.
type ErrSubscriptionRequired struct{}

func (e *ErrSubscriptionRequired) Error() string {
	return "subscription required to watch live video"
}
...
// IsSubscriptionRequired reports whether err is an ErrSubscriptionRequired.
func IsSubscriptionRequired(err error) bool {
	var target *ErrSubscriptionRequired
	return errors.As(err, &target)
}
