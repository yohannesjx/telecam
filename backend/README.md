...
### Phase 3A/3B: Parent invitation codes and Super App links

New tables added in migration `000015_parent_superapp_links`:

| Table | Purpose |
|-------|---------|
| `parent_superapp_links` | Permanent link record once parent redeems a code |
| `parent_invitation_codes` | Admin-generated one-time codes; raw code never stored |

New admin endpoints (authenticated as SUPER_ADMIN or SCHOOL_ADMIN):

```
POST /admin/parents/:parent_id/invitation-code      — generate code
GET  /admin/parents/:parent_id/invitation-codes     — list (no hash/raw code)
POST /admin/parents/:parent_id/invitation-codes/:code_id/revoke — revoke
GET  /admin/schools/:school_id/parent-links         — list confirmed links
```
...
|----------|----------|-------------|
| `PARENT_CODE_HMAC_SECRET` | production | HMAC-SHA256 key for hashing parent codes before storage. Dev falls back with a loud warning. |

The raw Parent Code (6-digit numeric, e.g. `493827`) is returned **once** in the generate response and never stored.

### Super App auth bridge (Phase 2)

Set these when you want parents authenticated via the Super App to call the
School Camera backend. Leave unset to skip the bridge routes entirely.

| Variable | Required | Description |
|----------|----------|-------------|
| `SUPERAPP_BACKEND_URL` | when bridge enabled | Base URL of the Super App backend — e.g. `http://superapp-api:17080`. No trailing slash. When empty, `/parent/superapp/*` routes are not registered. |
| `SUPERAPP_INTERNAL_TOKEN` | when bridge enabled | Shared secret sent as `X-Internal-Token` to `POST /internal/auth/introspect` on the Super App backend. Must match `INTERNAL_API_TOKEN` set there. |

Parent Super App endpoints (all require a Super App bearer token — validated via the introspect bridge):

```
GET  /parent/superapp/status   — returns linked status; if linked, includes parent_id, school_id, parent_name
POST /parent/superapp/link     — redeem an invitation code; atomically creates the link and marks the code used
```
...
**Error codes** (returned as `"code"` in the JSON body):

| Code | HTTP | Meaning |
|------|------|---------|
| `PARENT_CODE_INVALID` | 422 | Code not found, revoked, or otherwise invalid |
| `PARENT_CODE_EXPIRED` | 422 | Code has passed its 7-day expiry |
| `PARENT_CODE_ALREADY_USED` | 409 | Code was already redeemed |
| `PARENT_ALREADY_LINKED` | 409 | The parent account is already linked |
| `SUPERAPP_USER_ALREADY_LINKED` | 409 | This Super App user is already linked to a parent at this school |

### Phase 4 — Super App classes/cameras

```
GET /parent/superapp/classes
```

Auth: Super App bearer token + active `parent_superapp_links` record.

Returns children, classrooms, and camera metadata for the linked parent. Does **not** return playable stream URLs (no RTSP, no signed HLS).
...
`live_available: true` when `cameras.status = 'ACTIVE'`.
...
### Phase 5 — Super App live playback

```
GET /parent/superapp/cameras/:camera_id/live?quality=<quality>
```

Auth: Super App bearer token + active `parent_superapp_links` record.

Returns a signed HLS URL for the camera's live stream. Subject to the same school schedule and subscription checks as the legacy `/parent/cameras/:camera_id/live` endpoint. A trial subscription is created automatically at link time.

**Error codes:**

| Code | HTTP | Meaning |
|------|------|---------|
| `PARENT_LINK_REQUIRED` | 403 | No active parent link |
| `SUBSCRIPTION_REQUIRED` | 403 | No active subscription or trial |
| `LIVE_OUTSIDE_SCHOOL_HOURS` | 409 | Outside school schedule |
| `LIVE_TEMPORARILY_PAUSED` | 409 | Stream temporarily paused |

### Phase 6 — Super App billing summary
...
Returns subscription status, trial/renewal due date, amount due, and recent payment and invoice history. Does **not** perform any payment.
...
Subscription status values: `trial`, `active`, `expired`, `past_due`, `cancelled`, `blocked`, `none`.

Amount is a flat default (no per-school pricing in this phase). Configure via:
- `SCHOOL_CAMERA_MONTHLY_AMOUNT_MINOR` (default `50000`)
- `SCHOOL_CAMERA_CURRENCY` (default `ETB`)
...
- **Server `docker-compose.yml`:** Not tracked in git (see `docker-compose.example.yml`). Copy once per host: `cp docker-compose.example.yml docker-compose.yml`, then customize. `git pull` will never overwrite your file. Shared production tweaks belong in `docker-compose.prod.yml` (tracked).
